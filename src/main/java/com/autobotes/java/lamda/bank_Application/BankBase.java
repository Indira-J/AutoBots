...
public abstract class BankBase implements Bank{
    ...
    // приватные поля создали
    private long accountNumber;
    private long routingNumber;

    public static Set<BankBase> allBankRecords = new HashSet<>(); // что то типо базы данных

    public BankBase(long accountNumber, long routingNumber) { // конструктор для создания обьекта

        if(Long.toString(accountNumber).length() != 12) { // должно быть ровно 12 цифр
            throw new IllegalArgumentException("Account number must be 12 digits");
        } else if (Long.toString(routingNumber).length() != 9) { // должно быть ровно 9
            throw new IllegalArgumentException("Routing number must be 9 digits");
        } else {
            this.accountNumber = accountNumber; // если указыаем правильно сработает else
            this.routingNumber = routingNumber; // если указыаем правильно сработает else
        }
